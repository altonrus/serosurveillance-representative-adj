df_cbs_Roche_all <-
  ...
  filter(Province != ("YT") & Province !=("NU/NT") & Province != ("QC"), Province == ("BC"),
         sampledate >= '2020-12-31',sampledate <= '2021-01-31',
...
df_blca_on <- rbind(df_blca_cbs, df_blca_cbs_r)  
desired_order <- unique(df_blca_on$yearmon)
desired_order <- c('Dec 2020','Jan 2021')
desired_order1 <- c("obs", "boot", "obs_rg", "boot_rg")

# Reorder the levels of the 'category' variable
df_blca_on$yearmon <- factor(df_blca_on$yearmon, levels = desired_order)
df_blca_on$model <- factor(df_blca_on$model, levels = desired_order1)

compare_blca_1 <- ggplot(df_blca_on, aes(x = yearmon, y = sample_mean, colour=source, shape = model)) +
  theme_bw()+
  ...
  scale_color_manual(
    ...
    values = c("CBS" = "#00BFC4" ,
               "CBS_Abbott" = "#00BA38"),
    labels = c("CBS" = "CBS_Roche",
               "CBS_Abbott" = "CBS_Abbott")) +
  scale_x_discrete(guide = guide_axis(angle = 50)) +
  ylab("Seropositivity") +
  xlab("Month") +
  ggtitle("CBS,  Abbott/Roche, British Columbia") +
...
compare_blca <- ggplot(df_blca_on, aes(x = yearmon, y = sample_mean, colour=source, shape = model)) +
  theme_bw()+
  geom_errorbar(aes(ymin=lower, ymax=upper), width=0.3, position=pd_1) +
  geom_point(position=pd_1,  size = 2) +
  scale_color_manual(values = c("#00BFC4","#00BA38")) +
  #  geom_point(data = est_by_area, aes(x=cduid, y=cur_result_n), position=pd_1, colour = "violetred", size = 1) +
  scale_shape_manual(
    name = "Adjustment",
    values = c("obs" = 3, 
  ...
  ylab("Seropositivity") +
  xlab("Month") +
  ggtitle("CBS,  Abbott/Roche, Ontario") +
  theme(plot.title = element_text(vjust = 0))
compare_blca
